...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    # num_classes + 1 + 4  4代表调整参数，1代表是否包含物体
...
def yolo_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape):
    ...
    # 获得得分和box
    boxes = K.reshape(boxes, [-1, 4])# 代表每一个框 的四个参数
    box_scores = box_confidence * box_class_probs
    box_scores = K.reshape(box_scores, [-1, num_classes])# 第一个维度代表每一个框，带二个维度代表每个框的种类的置信度
...
def yolo_eval(yolo_outputs,
    ...
    # 特征层2对应的anchor是345
    # 特征层3对应的anchor是012
    anchor_mask = [[6,7,8], [3,4,5], [0,1,2]]# 对特征层的先验框进行筛选，每个特征层先验框不一样
    ...
    for c in range(num_classes):
        ...
        #tf.image.non_max_suppression这个函数的功能是在检测算法中我们的一个目标被检测了多次，
        # 如何排除掉多余的边界框。
        # 下面的这个车被多次检测到，存在多个边界框，保留概率最大的那个，去除掉与这个概率最大的边界框的IoU大于一个阙值的其余边界框。
        # 这个过程就成为非最大值抑制=NMS
        # 返回值：代表的是选出来的留下来的边框下标
        ...
        classes = K.ones_like(class_box_scores, 'int32') * c# K.ones_like该操作返回一个具有和给定tensor相同形状（shape）和相同数据类型（dtype）
